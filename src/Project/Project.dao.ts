...
import { IProjectReturn } from "./projectInterface";
...
class ProjectDao {
  static async createProject(title: string): Promise<IProjectReturn> {
    ...
    return {
      ...
      access_tokens: project.access_tokens.map((e) => {
        return {
          token: e.token,
  ...
  static async getAllProjects(userID:string = "123"):Promise<IProjectReturn[]>{
    ...
    const projectRefactor = projects.map((p) => ({
      ...
      access_tokens: p.access_tokens.map((t: any) => {
        return {
          token: t.token,
  ...
  static async updateProject(id: string, title: string) {
    const project = await Project.findById(id);
    if(!project) new FluidError("Project not found",404);
    project!.title = title;
    await project!.save();
